...
object HouseholdActor {
  ...
  case class HouseholdAttributes(householdIncome: Double,
                                 householdSize: Int,
                                 numCars: Int,
                                 numBikes: Int)

  case class AttributesOfIndividual(person: Person,
  ...
  class HouseholdActor(
    ...
    household.members.foreach { person =>
      ...
      val matsimBodyVehicle = VehicleUtils.getFactory
        .createVehicle(bodyVehicleIdFromPerson, MatsimHumanBodyVehicleType)
      ...
      val newBodyVehicle = new BeamVehicle(powerTrainForHumanBody(),
                                           matsimBodyVehicle,
                                           None,
                                           HumanBodyVehicle)
    ...
    /**
      ...
      */
    var _vehicles: Vector[Id[Vehicle]] =
    ...
    /**
      ...
      */
    var _pendingInquiries: Map[Id[MobilityStatusInquiry], Id[Vehicle]] =
    ...
    /**
      ...
      */
    var _reservedForPerson: mutable.Map[Id[Person], Id[Vehicle]] =
      mutable.Map[Id[Person], Id[Vehicle]]()

    /**
      * Vehicles that are currently checked out to traveling agents.
      */
    var _checkedOutVehicles: mutable.Map[Id[Vehicle], Id[Person]] =
      ...
      case ReleaseVehicleReservation(personId, vehId) =>
        ...
        _reservedForPerson.get(personId) match {
          case Some(vehicleId) if vehicleId == vehId =>
            log.debug(
              s"Vehicle $vehicleId is now available for anyone in household $id")
    ...
    private def initializeHouseholdVehicles(): Unit = {
      ...
      for (i <- _vehicles.indices.toSet ++ household.rankedMembers.indices.toSet) {
        if (i < _vehicles.size & i < household.rankedMembers.size) {
          _reservedForPerson = _reservedForPerson + (household
            .rankedMembers(i)
