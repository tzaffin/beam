...
class PersonAgent(val scheduler: ActorRef,
          ...
          goto(ChoosingMode) using ChoosesModeData(
            personData = data.copy(
              ...
              currentTourMode = data.currentTourMode.orElse(
                _experiencedBeamPlan.getPlanElements.get(
  ...
  when(WaitingForDeparture) {
    ...
    case Event(TriggerWithId(PersonDepartureTrigger(tick), triggerId),
  ...
  when(WaitingForTransitReservationConfirmation) {
    ...
    case Event(ReservationResponse(_, Left(error)), data: BasePersonData) =>
      ...
      log.warning("at {} replanning leg {} because {}",
                  tick,
                  data.restOfCurrentTrip.head,
                  error.errorCode)
      holdTickAndTriggerId(tick, triggerId)
  ...
  when(Waiting) {
     ...
     */
    case Event(
        TriggerWithId(NotifyLegStartTrigger(_, beamLeg), triggerId),
        BasePersonData(_, _, currentLeg :: _, currentVehicle, _, _, _, _, _))
        if beamLeg == currentLeg.beamLeg && currentLeg.beamVehicleId == currentVehicle.head =>
      logDebug(s"Already on vehicle: ${currentVehicle.head}")
      goto(Moving) replying CompletionNotice(triggerId)

    case Event(TriggerWithId(NotifyLegStartTrigger(tick, beamLeg), triggerId),
               data @ BasePersonData(_,
                                     ...
                                     _,
                                     _,
                                     _)) if beamLeg == currentLeg.beamLeg =>
      eventsManager.processEvent(
        new PersonEntersVehicleEvent(
          tick,
          id,
          Id.createVehicleId(currentLeg.beamVehicleId)))
      goto(Moving) replying CompletionNotice(triggerId) using data.copy(
        currentVehicle = Id
          .createVehicleId(currentLeg.beamVehicleId) +: currentVehicle)
  }
  ...
  when(Moving) {
     ...
     */
    case Event(TriggerWithId(NotifyLegEndTrigger(_, beamLeg), triggerId),
               data @ BasePersonData(_,
                                     _,
                                     currentLeg :: restOfCurrentTrip,
                                     currentVehicle,
                                     _,
                                     _,
                                     _,
                                     _,
                                     _))
        if beamLeg == currentLeg.beamLeg && restOfCurrentTrip.head.beamVehicleId == currentVehicle.head =>
    ...
    case Event(TriggerWithId(NotifyLegEndTrigger(tick, beamLeg), triggerId),
               data @ BasePersonData(_,
                                     _,
                                     currentLeg :: restOfCurrentTrip,
                                     currentVehicle,
      ...
      eventsManager.processEvent(
        new PersonLeavesVehicleEvent(tick, id, currentVehicle.head))
      holdTickAndTriggerId(tick, triggerId)
  ...
  when(ProcessingNextLegOrStartActivity, stateTimeout = Duration.Zero) {
    ...
    case Event(StateTimeout,
               data @ BasePersonData(currentActivityIndex,
                                     Some(currentTrip),
                                     _,
                                     _,
                                     currentTourMode,
