...
trait ChoosesMode {
  ...
  onTransition {
    case (PerformingActivity | Waiting |
        WaitingForTransitReservationConfirmation) -> ChoosingMode =>
    ...
    transform {
      case Event(MobilityStatusReponse(streetVehicles),
        ...
        val rideHailingResult =
          choosesModeData.personData.currentTourMode match {
            ...
            case _ =>
              Some(
                RideHailingInquiryResponse(
                  Id.create[RideHailingInquiry]("NA",
                                                classOf[RideHailingInquiry]),
                  Vector(),
                  Some(RideHailNotRequestedError)))
          }

        def makeRequestWith(transitModes: Vector[BeamMode],
                            vehicles: Vector[StreetVehicle],
                            streetVehiclesAsAccess: Boolean = true): Unit = {
          router ! RoutingRequest(
            ...
            vehicles,
            streetVehiclesAsAccess)
        }

        def makeRideHailRequest(): Unit = {
          rideHailingManager ! RideHailingInquiry(
            RideHailingManager.nextRideHailingInquiryId,
            id,
            currentActivity(choosesModeData.personData).getCoord,
            departTime,
            nextAct.getCoord)
        ...
        def filterStreetVehiclesForQuery(
            ...
            byMode: BeamMode): Vector[StreetVehicle] = {
        ...
        choosesModeData.personData.currentTourMode match {
          case None =>
            makeRequestWith(Vector(TRANSIT),
                            streetVehicles :+ bodyStreetVehicle)
            makeRideHailRequest()
          case Some(WALK) =>
          ...
          case Some(mode @ (CAR | BIKE)) =>
            val maybeLeg = _experiencedBeamPlan.getPlanElements.get(
              _experiencedBeamPlan.getPlanElements.indexOf(nextAct) - 1) match {
              case l: Leg => Some(l)
              case _      => None
            }
        ...
        stay() using choosesModeData.copy(availablePersonalStreetVehicles =
                                            availablePersonalStreetVehicles,
                                          rideHailingResult = rideHailingResult)
      /*
      ...
      case Event(theRideHailingResult: RideHailingInquiryResponse,
                 choosesModeData: ChoosesModeData) =>
        stay() using choosesModeData.copy(
  ...
  when(WaitingForReservationConfirmation)(transform {
    case Event(response @ ReservationResponse(_, _),
               choosesModeData: ChoosesModeData) =>
      if (response.response.isRight) {
        ...
        if (choosesModeData.routingResponse.get.itineraries.isEmpty & choosesModeData.rideHailingResult.get.error.isDefined) {
          ...
          goto(ChoosingMode) using choosesModeData.copy(
            ...
            rideHailingResult = Some(
              choosesModeData.rideHailingResult.get
                .copy(proposals = Vector(), error = Some(firstErrorResponse))),
  ...
  def completeChoiceIfReady: PartialFunction[State, State] = {
    case FSM.State(_,
                   choosesModeData @ ChoosesModeData(personData,
                                                     None,
                                                     Some(routingResponse),
                                                     Some(rideHailingResult),
                                                     _,
                                                     _),
                   _,
                   _,
                   _) =>
      val nextAct = nextActivity(choosesModeData.personData).right.get
      val combinedItinerariesForChoice = rideHailingResult.proposals.flatMap(
        x =>
          x.responseRideHailing2Dest.itineraries) ++ routingResponse.itineraries
      ...
      modeChoiceCalculator(filteredItinerariesForChoice) match {
        case Some(chosenTrip)
            if RideHailingAgent.getRideHailingTrip(chosenTrip).nonEmpty =>
          val rideHailingLeg = RideHailingAgent.getRideHailingTrip(chosenTrip)
          val departAt = DiscreteTime(
            rideHailingLeg.head.beamLeg.startTime.toInt)
          rideHailingManager ! ReserveRide(
            choosesModeData.rideHailingResult.get.inquiryId,
            VehiclePersonId(bodyId, id),
            currentActivity(personData).getCoord,
            departAt,
            nextActivity(personData).right.get.getCoord
          )
          goto(WaitingForReservationConfirmation) using choosesModeData.copy(
        ...
        case None =>
          ...
          val originalWalkTripLeg = routingResponse.itineraries
            .filter(_.tripClassifier == WALK)
            .head
            .legs
            .head
  ...
  when(FinishingModeChoice, stateTimeout = Duration.Zero) {
    case Event(StateTimeout, data: ChoosesModeData) =>
      ...
      if (links.nonEmpty) {
        ...
        _experiencedBeamPlan
          .activities(data.personData.currentActivityIndex)
          .setLinkId(Id.createLinkId(beamServices.geo
            .getNearestR5Edge(transportNetwork.streetLayer, origin, 10000)))
        _experiencedBeamPlan
          .activities(data.personData.currentActivityIndex + 1)
          .setLinkId(
            Id.createLinkId(
      ...
      if (chosenTrip.tripClassifier != RIDE_HAIL && data.rideHailingResult.get.proposals.nonEmpty) {
        rideHailingManager ! ReleaseVehicleReservation(
          id,
          data.rideHailingResult.get.proposals.head.rideHailingAgentLocation.vehicleId)
      }
