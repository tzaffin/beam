...
class ModeChoiceMultinomialLogit(val beamServices: BeamServices,
  ...
  override def apply(
    ...
    if (alternatives.isEmpty) {
      ...
      val groupedByMode =
        modeCostTimeTransfers.sortBy(_.mode.value).groupBy(_.mode)

      val bestInGroup = groupedByMode.map {
        case (mode, modeCostTimeSegment) =>
  ...
  def utilityOf(mode: BeamMode,
    ...
    val variables = Map("transfer" -> numTransfers.toDouble,
                        "cost" -> cost.toDouble,
                        "time" -> time)
  ...
  override def utilityOf(alternative: EmbodiedBeamTrip): Double = {
    ...
    utilityOf(modeCostTimeTransfer.mode,
  ...
  def altsToModeCostTimeTransfers(
      alternatives: Seq[EmbodiedBeamTrip]): Seq[ModeCostTimeTransfer] = {
    ...
    alternatives.zipWithIndex.map { altAndIdx =>
      val totalCost = altAndIdx._1.tripClassifier match {
        case TRANSIT | WALK_TRANSIT | DRIVE_TRANSIT =>
          (altAndIdx._1.costEstimate + transitFareDefaults(altAndIdx._2)) * beamServices.beamConfig.beam.agentsim.tuning.transitPrice + gasolineCostDefaults(
            altAndIdx._2) + bridgeTollsDefaults(altAndIdx._2)
        case RIDE_HAIL =>
      ...
      val numTransfers = altAndIdx._1.tripClassifier match {
        case TRANSIT | WALK_TRANSIT | DRIVE_TRANSIT =>
          ...
          altAndIdx._1.legs.foreach { leg =>
            if (BeamMode
                  .withValue(leg.beamLeg.mode)
                  .isTransit() && leg.beamVehicleId != vehId) {
      ...
      ModeCostTimeTransfer(altAndIdx._1.tripClassifier,
                           totalCost,
...
object ModeChoiceMultinomialLogit {
  ...
  case class ModeCostTimeTransfer(mode: BeamMode,
                                  cost: BigDecimal,
                                  time: Double,
                                  numTransfers: Int,
                                  index: Int = -1)
